...
class PySysTest(ApamaBaseTest):
	def execute(self):
		...
		correlator.flush() 
		self.waitForSignal(file='testcorrelator.log', expr='Round-trip OK', condition='>=200')
	...
	def validate(self):
		...
		self.assertLineCount('before.txt', expr='Towards Host: {sag.type:Data,sag.channel:batchChain} / {s:Hello World}', condition='==100')
		self.assertLineCount('before.txt', expr='Towards Transport: {sag.type:Data,sag.channel:batchChain} / {s:Hello World}', condition='==100')
		self.assertGrep('after.txt', expr='Towards Transport: {sag.type:Data,sag.channel:batchChain} / \\[', condition='>=1')
		self.assertGrep('after.txt', expr='Towards Host: {sag.type:Data,sag.channel:batchChain} / \\[{s:Hello World},{s:Hello World}', condition='>=1')
