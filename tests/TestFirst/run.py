...
class PySysTest(ApamaBaseTest):
	...
	def validate(self):
		...
		self.assertGrep('after.txt', expr='Towards Transport: {sag.type:Data,sag.channel:batchChain} / \\[', contains=True)
		self.assertGrep('after.txt', expr='Towards Host: {sag.type:Data,sag.channel:batchChain} / \\[{s:Hello World},{s:Hello World}', contains=True)
